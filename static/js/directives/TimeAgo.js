kitin.directive('timeAgo', ['TickNotificationService','$filter',
function(TickNotificationService, $filter) {
    return {
        template: '<span title="{{modifiedFormated}}">{{timeAgo}}</span>',
        replace: true,
        link: function(scope, element, attrs) {
            // "Inspired by" http://stackoverflow.com/a/12475270
            function changeTimeFormat(time, verboseAfter, verboseFormat) {
                if(time) {
            ...
            scope.modifiedFormated = changeTimeFormat(scope.$eval(attrs.timeAgo), -1);
            var updateTime = function() {
                scope.timeAgo = changeTimeFormat(scope.$eval(attrs.timeAgo), scope.$eval(attrs.verboseAfter), scope.$eval(attrs.verboseFormat));
            };
            TickNotificationService.onTimeAgo(scope, updateTime); // subscribe
            updateTime();
