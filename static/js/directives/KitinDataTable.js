    ...
    compile: function(element, attrs) {
      ...
      '<table ng-if="' + attrs.tableModel + '.length > 0">' +
          ...
          '<tfoot ng-if="' + (typeof attrs.addable !== 'undefined') + '">' +
              '<tr>' +
                '<td>' +
                  '<button class="add-thing btn-link" data-ng-click="expand();addObject(' + attrs.subject + ', \'' +  attrs.linkMultiple + '\',\'' + type + '\',\'' + attrs.ngTarget + '\',\'' + attrs.ngSwitchWhen + '\')">Lägg till</button>' +
                '</td>' +
              '</tr>' +
            '</tfoot>' +
        '</table>' + 
